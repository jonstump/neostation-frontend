...
`RommService.runTask(name)` SHALL `POST /api/tasks/run/{name}` and return the queued task id. The failure kind MUST be read from the response **body**, not from the status alone: a body saying the task is already running, queued, or in progress MUST map to `RommErrorKind.taskBusy` whatever status carried it, and a status code on its own MUST NOT. A 403 MUST settle as `RommErrorKind.scopeDenied` **before** the body is consulted — the server named that reason itself, and an intermediary error page repeating the words "already running" must not turn a permission failure into a busy one. Every other failure, a bare 400 included, MUST map to `RommErrorKind.other` and be reported with `rommMaintenanceFailed`. The response body MUST be logged on failure so an unexpected status is diagnosable rather than guessed at. (Amended after issue #170. This clause read "400 or a \"already running\" answer MUST map to `RommErrorKind.taskBusy`", and the implementation short-circuited on the status so a 400 never reached the body. RomM 5.1.0 documents only 200 and 422 for this route, so the 400 was undocumented and reading it as "busy" was a guess: a user whose task was refused for a real reason — a task name dropped by a RomM upgrade, a server precondition, a proxy answering first — was told the work was already underway and waited for it.) The connected RomM screen's header menu SHALL offer "Server maintenance" with "Rescan library" (`scan_library`), "Sync folder scan" (`sync_folder_scan`), and "Clean up missing ROMs" (`cleanup_missing_roms`), each confirmed, shown only when `hasScope(tasksRun)` is **`granted`**, reporting queued or busy.
