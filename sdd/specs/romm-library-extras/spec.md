...
Both REQs previously said "granted". The consequence of the strict gate is deliberate and worth naming: `configure()` resets every group to `unknown` and only a fresh password grant negotiates them, so **token-restored sessions and every API-key connection — which is what ADR-0007's pair-code and QR logins mint — never see the maintenance menu.** Those users run maintenance from RomM's own web UI.
