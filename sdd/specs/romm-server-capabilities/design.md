...
- ~~The `playSessions` threshold is inferred from the ingest commit date (2026-03-22, between 4.7.0 and 4.8.0); the implementer SHOULD confirm 4.8.0 against the RomM release notes and record it in the table comment.~~ **Resolved, and the method here was wrong.** The commit date placed it in 4.8.0; the commit is not an ancestor of 4.8.0 or 4.8.1, only of 4.9.0, and the endpoint and model files are absent from both 4.8.x trees. The threshold is **4.9.0**. Inferring a threshold from a commit date is what produced this error and the `randomRom` 4.8.0-vs-5.2.0 error, so SPEC-0010 REQ "Feature Threshold Table" now requires a check of the source tree at the release tag instead. Release notes are not sufficient either — they say what was announced, not what shipped in the tag.
