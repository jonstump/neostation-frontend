...
The system SHALL define `RommFeature` with one entry per gated endpoint and the RomM version that introduced it: `playSessions` 4.9.0, `clientTokenExchange` 4.8.0, `romLookupByHash` 4.5.0. `RommServerCapabilities.supports(feature)` MUST return `supported` when the version is at or above the threshold, `unsupported` when below, and `unknown` when there is no version. A prerelease of the threshold version MUST count as below it. Each entry MUST carry a comment naming the RomM release it was verified against, and the verification MUST be a check of the source tree **at that release tag** — that the endpoint or field is absent at the tag below and present at the tag recorded. A commit date MUST NOT be used, and a commit MUST NOT be assumed to be in a release because it predates the tag: `playSessions` was recorded as 4.8.0 on that reasoning and was actually 4.9.0, because the commit is not an ancestor of 4.8.0 at all, and `randomRom` was recorded as 4.8.0 and was actually 5.2.0. Both errors ship a control that calls an endpoint the server does not have. The table MUST live in one file.
...
- **WHEN** the version is 4.9.0-beta.1 and the feature is `playSessions`
- **THEN** `supports` returns `unsupported`
...
`authenticate()` SHALL call `fetchHeartbeat()` first when the connection has not been probed since the last `configure()`. When `supports(playSessions)` is `unsupported`, the password grant MUST request only the read scopes and MUST NOT retry with the playtime scopes; the playtime-scope-granted flag MUST be false. When `supported` or `unknown`, the grant and its 403 fallback MUST behave as SPEC-0013 REQ "Optional Scope Groups" defines. (Until SPEC-0013, that was the binary playtime grant with a single 403 retry; ADR-0013 `extends` ADR-0010 and replaced it with per-group negotiation. This clause originally read "exactly as before this spec", which is stale now that SPEC-0013 has superseded the fallback it referred to.) API-key mode MUST still probe (the version still gates endpoints, and since the amended SPEC-0013 REQ "Optional Scope Groups" the same call also settles the scope groups). A connection restored by `RommProvider.initialize()` never calls `authenticate()` — the restore is deliberately offline — so it MUST verify and probe lazily, before its first authenticated request rather than during the restore. The verification is bounded rather than strictly single: a credential the server **rejects** (401/403) MUST NOT be retried for the life of the connection; a request the server **answers badly** (any other non-200) MAY be retried, and MUST be capped so a permanently failing server cannot cost a probe on every call; a request the server **does not answer** at all MUST be retried, because a handheld routinely issues its first request before its network is up and latching there reproduces the very failure this clause was added for. The unanswered case MUST be identified by how the failure was raised rather than by the absence of a status code, and MUST include every shape a dropped connection takes — timeout, socket, TLS, and a client-library error standing in for a closed connection. A fourth case exists for a failure that reaches no server and is not a dropped connection either — an unset URL, a malformed response — which MAY be bounded like a badly-answered request, since retrying cannot fix it. (The fourth case was added after issue #183: classifying by "has no status code" swept genuine transport failures together with programming errors, and the two want opposite treatment.) This clause originally read "at most once", which the implementation exceeded from the day the re-arm shipped. (Added after issue #168. ADR-0010 already claimed a restored session re-probes "off the critical path"; it did not, so a resumed session ran with null capabilities and every scope group `unknown` for the life of the process. That made version-gated controls render on servers that cannot serve them — Surprise Me was visible against a 5.1.0 server and answered 422 — and left `granted` unreachable.)
