...
1. **Provenance column.** Add `link_source TEXT` to `app_romm_rom_map` by a versioned migration, nullable, with values `download`, `auto`, and `manual`. Existing rows backfill to `download` when they carry a `romm_fs_name` written by the download path and are otherwise left null, which reads as automatic. Only `manual` changes behaviour.
