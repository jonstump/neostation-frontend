...
1. **An injectable RomM step.** A `RommScrapeStep` value, built by `RommProvider` when it is connected and `null` otherwise, that given a scrape target (system id, filename with extension, system folder, overwrite flag) returns a `RommScrapeStepResult` whose status is one of scraped, not linked, not found, empty, or failed (with the writer's `RommMetadataOutcome` attached when a fetch ran). `ScreenScraperService.scrapeSingleGame` and `startMetadataScraping` accept the step as an optional parameter. When the step is `null` nothing changes.
