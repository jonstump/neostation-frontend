...
3. **Media mirror.** Copy the files in the mapped category folders (the same category set ADR-0002 defines) into `<user data>/imported_media/<system folder>/<category>/`, streamed in Dart over the existing SAF `readRange` channel (the spec's design chose the Dart loop over a native mirror), skipping files whose size already matches the destination and reporting progress. Unmapped folders are not copied. The mirror directory, not the platform folder, is recorded as the system's `esde_media_root`.
