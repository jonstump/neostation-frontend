...
1. **Source.** `RetroArchConfigService` also parses `screenshot_directory` (and `sort_screenshots_by_content_enable`); the collector lists files there whose name starts with any of the game's content stems — the ROM filename stem, and for a `.zip` the stem of its largest member, because RetroArch names captures after the content it loaded — and whose mtime is at or after the session start. Standalone emulators are out of scope until their directories are known.
