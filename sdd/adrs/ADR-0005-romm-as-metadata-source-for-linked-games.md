...
3. **Provenance column.** `user_screenscraper_metadata.metadata_source TEXT` by a versioned migration, values `screenscraper`, `romm`, `esde`, `manual`, null for legacy rows. Every writer sets it; fill-gaps writes set it only on insert, mirroring `esde_imported`. This is what makes "which games has RomM already filled" and a per-source badge possible.
