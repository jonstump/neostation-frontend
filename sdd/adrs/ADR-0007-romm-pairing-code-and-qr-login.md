...
3. **A third auth mode on the connect screen.** The mode switch gains "Pairing code" beside password and API key: server URL plus a code field that accepts `XXXX-XXXX` or `XXXXXXXX`, with a hint that the code lasts about a minute, and Connect exchanging immediately. The switch stays D-pad operable (Left/Right step between the three modes and stop at the ends; A advances and wraps).
