...
1. **Filters and random.** `getRomsPage` gains the boolean filters; `RommProvider` holds a filter set per platform or collection; the browse screen gets a filter menu (Y on the header: favourites, with saves, with states, with achievements, playable, duplicates, missing files) whose active filters show as chips and reset on leaving the platform; "Surprise me" calls `/api/roms/random` scoped to the current platform or collection (gated 4.8.0) and focuses the result.
...
flowchart LR
    ...
    B --> RND["Surprise me → /api/roms/random (4.8.0)"]
...
* RomM: filter names on `GET /api/roms`; `random` since 4.8.0; `search/roms`, `search/cover` (3.10 or earlier); `PUT /api/roms/{id}` multipart fields; `tasks/registry.py` names.
