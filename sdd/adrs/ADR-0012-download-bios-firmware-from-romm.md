...
2. **Destination.** The system's BIOS destination is the RetroArch `system_directory` when the RetroArch config is known. When it is not, the panel asks the user to pick a BIOS folder once and stores it in `user_config.bios_directory` (versioned, guarded migration). A per-system override is out of scope; the emulator JSON may grow a `bios_dir` later.
