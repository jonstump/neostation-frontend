...
4. **Provider and UI.** `RommProvider` re-probes on `connect`, `connectWithPairCode`, and `initialize` (restored session, off the critical path), and exposes the version and the `FRONTEND.DISABLE_USERPASS_LOGIN` flag. The connect screen shows the server version and, when password login is disabled on the server, leads with pairing and API-key modes. The pairing flow reports "this server is too old for pairing" instead of a raw 404.
