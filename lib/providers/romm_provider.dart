...
class RommProvider extends ChangeNotifier {
    ...
    for (final folder in romFolders) {
      ...
      for (final name in _systemFolderNames(system)) {
        ...
        if (system.folderName == 'scummvm') {
          final found = await _findNamedFileRecursively(dir, candidates);
          if (found != null) {
            return RommLocalCopy(
              system: system,
              directory: dir,
              filename: p.basename(found.path),
            );
          }
        }
      }
  ...
  /// The file under [dir] carrying one of [names], anywhere below it.
  ///
  /// Returns the file rather than a bool because the caller needs the name the
  /// library scan recorded: a ScummVM game's data sits in an ID-named
  /// subfolder, so its descriptor is not a direct child of the system folder
  /// and the candidate spelling is not what ends up indexed.
