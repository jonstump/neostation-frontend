...
const Map<String, dynamic> appLocalePt = {
  ...
  AppLocale.rommSettings: 'Configurações do RomM',
  ...
  AppLocale.rommSyncConfirmLinked: '{count} vinculados ao RomM.',
  ...
  AppLocale.inFolderImport: 'Importar metadados das pastas de ROMs',
  AppLocale.inFolderImportSubtitle:
      'Usar os arquivos gamelist.xml e as imagens encontrados dentro das suas pastas de ROMs',
  AppLocale.inFolderImporting: 'Importando das pastas de ROMs',
  AppLocale.inFolderImportComplete: 'Importação das pastas de ROMs concluída',
  AppLocale.inFolderImportNoGamelists:
      'Nenhum arquivo gamelist.xml foi encontrado nas suas pastas de ROMs',
  AppLocale.inFolderImportFoldersSkippedSaf:
      '{count} pastas de ROMs foram ignoradas porque não são acessíveis como caminhos reais (acesso a todos os arquivos necessário no Android)',
  AppLocale.inFolderSummarySystemsFound: 'Sistemas encontrados',
  AppLocale.inFolderSummaryMediaOnlyLinked:
      'Sistemas somente com mídia vinculados',
  AppLocale.inFolderSummaryFoldersSkippedSaf:
      'Pastas ignoradas (sem acesso ao caminho real)',
  ...
  AppLocale.rommLinkPickerUnscoped:
      'Nenhuma plataforma do RomM corresponde a este sistema, então todas as plataformas são pesquisadas.',
  ...
  AppLocale.rommLinkPickerCleanedQuery: 'Mostrando resultados para “{query}”',
  ...
  AppLocale.rommFetchMetadataRowSubtitle:
      'Preencher ou substituir os metadados e as imagens deste jogo a partir do RomM',
  AppLocale.rommFetchMetadataRowNotLinked: 'Vincule este jogo ao RomM primeiro',
  AppLocale.rommFetchMetadataAction: 'Obter',
  AppLocale.rommFetchMetadataChooserTitle: 'Obter metadados do RomM',
  AppLocale.rommFetchMetadataFillGapsOption: 'Preencher lacunas (recomendado)',
  AppLocale.rommFetchMetadataFillGapsDescription:
      'Grava apenas os campos vazios e as imagens ausentes. Tudo o que você já tem é mantido.',
  AppLocale.rommFetchMetadataReplaceOption: 'Substituir tudo',
  AppLocale.rommFetchMetadataReplaceDescription:
      'Sobrescreve todos os campos e imagens com os dados do RomM. As descrições que não estão em inglês são apagadas e a editora fica vazia, pois o RomM não os fornece.',
  AppLocale.rommFetchMetadataInProgress: 'Obtendo do RomM…',
  AppLocale.rommFetchMetadataFilled:
      '{count} campos e {media} imagens preenchidos a partir do RomM',
  AppLocale.rommFetchMetadataReplaced:
      '{count} campos e {media} imagens substituídos a partir do RomM',
  AppLocale.rommFetchMetadataNothingToFill:
      'Nada a preencher: este jogo já tem tudo o que o RomM fornece',
  AppLocale.rommFetchMetadataNotFound:
      'Não encontrado no RomM: a ROM vinculada não existe mais lá',
  ...
  AppLocale.rommSystemFetchRequiresConnection:
      'Conecte-se ao RomM para ativar esta ação',
  AppLocale.rommSystemFetchRowRunning:
      'Obtendo {done}/{total} — pressione para cancelar',
  AppLocale.rommSystemFetchChooserTitle:
      'Obter metadados do RomM para {system}',
  AppLocale.rommSystemFetchModeFillGaps: 'Preencher lacunas (recomendado)',
  AppLocale.rommSystemFetchModeFillGapsHint:
      'Apenas campos vazios e artes ausentes são gravados. Os dados existentes são mantidos.',
  AppLocale.rommSystemFetchModeReplace: 'Substituir tudo',
  AppLocale.rommSystemFetchModeReplaceWarning:
      'Sobrescreve todos os campos e arquivos de arte. Descrições em idiomas diferentes do inglês são apagadas e o RomM não fornece editora.',
  AppLocale.rommSystemFetchStarted:
      'Obtendo metadados do RomM para {system} em segundo plano',
  AppLocale.rommSystemFetchPreparing:
      'Preparando a obtenção de metadados do RomM para {system}…',
  AppLocale.rommSystemFetchProgress:
      'Obtendo metadados do RomM para {system}: {done}/{total}',
  AppLocale.rommSystemFetchSummary:
      'Metadados do RomM para {system}: {linked} vinculados, {filled} preenchidos, {replaced} substituídos, {unlinked} não vinculados ignorados, {notFound} não encontrados, {failed} com falha',
  AppLocale.rommSystemFetchCancelled: 'Cancelado — {summary}',
  ...
  AppLocale.scrapeSummaryFromRomm: 'Do RomM: {count}',
  AppLocale.scrapeSummaryFromScreenscraper: 'Do ScreenScraper: {count}',
  ...
  AppLocale.rommPairCodeHint:
      'No RomM abra Configurações → Client API Tokens → Parear dispositivo. O código expira cerca de um minuto após ser gerado.',
  AppLocale.rommPairCodeInvalidLength:
      'Informe a URL do servidor e o código de pareamento de 8 caracteres (XXXX-XXXX)',
  AppLocale.rommPairCodeInvalid:
      'Esse código de pareamento não é válido; confira os 8 caracteres',
  AppLocale.rommPairCodeExpired:
      'Código de pareamento expirado ou já usado; gere um novo no RomM',
  AppLocale.rommPairRateLimited:
      'Muitas tentativas; aguarde um minuto e tente novamente',
  AppLocale.rommPairedTokenExpires: 'O token expira em {date}',
  AppLocale.rommPairedTokenName: 'Token: {name}',
  AppLocale.rommScanQrAction: 'Ler código QR',
  AppLocale.rommScanQrTitle: 'Ler código de pareamento',
  AppLocale.rommScanQrPrompt:
      'Aponte a câmera para o código QR de pareamento mostrado no RomM',
  AppLocale.rommScanQrNotPairingCode:
      'Isso não é um código de pareamento do RomM',
  AppLocale.rommScanQrCameraDenied:
      'A permissão da câmera foi negada; digite o código em vez disso',
  AppLocale.rommScanQrCameraUnavailable:
      'Nenhuma câmera disponível; digite o código em vez disso',
  ...
  AppLocale.rommSyncConfirmCollection:
      'A coleção «{name}» será criada ou atualizada no NeoStation.',
  AppLocale.rommSyncOutcomeCollection: 'Coleção «{name}»: {count} jogos',
  AppLocale.rommSyncOutcomeCollectionFailed:
      'Não foi possível atualizar a coleção «{name}»',
  AppLocale.collectionRommMirrored: 'Sincronizada do RomM',
  AppLocale.collectionUnlinkRomm: 'Desvincular do RomM',
  AppLocale.collectionUnlinkRommConfirm:
      'Esta coleção deixará de seguir a coleção do RomM. Os seus jogos permanecem.',
  AppLocale.rommPairServerTooOld:
      'Este servidor RomM é antigo demais para o emparelhamento (requer 4.8.0 ou mais recente)',

  // RomM firmware/BIOS panel (SPEC-0012)
  AppLocale.rommFirmwareRowTitle: 'Arquivos de BIOS do RomM',
  AppLocale.rommFirmwareRowSubtitle:
      'Veja de quais arquivos de BIOS este sistema precisa e baixe os que faltam',
  AppLocale.rommFirmwareRowRequiresConnection:
      'Conecte-se ao RomM para gerenciar os arquivos de BIOS',
  AppLocale.rommFirmwarePanelTitle: 'Arquivos de BIOS de {system}',
  AppLocale.rommFirmwareLoading: 'Carregando arquivos de BIOS…',
  AppLocale.rommFirmwareEmpty:
      'O RomM não tem arquivos de BIOS para esta plataforma',
  AppLocale.rommFirmwareScopeDenied:
      'Esta conta do RomM não tem acesso ao firmware',
  AppLocale.rommFirmwareListFailed:
      'Não foi possível carregar os arquivos de BIOS: {error}',
  AppLocale.rommFirmwareStatePresent: 'Presente',
  AppLocale.rommFirmwareStateMissing: 'Faltando',
  AppLocale.rommFirmwareStateServerMissing: 'Não armazenado no servidor',
  AppLocale.rommFirmwareStateNoFolder: 'Nenhuma pasta de BIOS escolhida',
  AppLocale.rommFirmwareVerifiedByServer: 'Verificado pelo RomM',
  AppLocale.rommFirmwareVerifyChecking: 'Verificando…',
  AppLocale.rommFirmwareVerifyMatch: 'Igual à cópia do servidor',
  AppLocale.rommFirmwareVerifyMismatch: 'Diferente da cópia do servidor',
  AppLocale.rommFirmwareVerifyUnreadable:
      'Não foi possível ler o arquivo local',
  AppLocale.rommFirmwareActionDownload: 'Baixar',
  AppLocale.rommFirmwareActionVerify: 'Verificar',
  AppLocale.rommFirmwareActionDownloadAll: 'Baixar todos os que faltam',
  AppLocale.rommFirmwareActionChooseFolder: 'Escolher pasta de BIOS',
  AppLocale.rommFirmwareDestination: 'Destino: {path}',
  AppLocale.rommFirmwareDestinationMissing:
      'Escolha uma pasta de BIOS antes de baixar',
  AppLocale.rommFirmwareFolderFailed:
      'Essa pasta não pode ser usada para arquivos de BIOS',
  AppLocale.rommFirmwareDownloadProgress: 'Baixando {file} ({done}/{total})',
  AppLocale.rommFirmwareDownloadSummary:
      'Arquivos de BIOS: {downloaded} baixados, {failed} com falha',
  ...
  AppLocale.rommFirmwareDestinationRetroArch: 'Destino (do RetroArch): {path}',
  AppLocale.rommFilterMenuTitle: 'Filtros',
  AppLocale.rommFilterMenuAction: 'Filtros',
  AppLocale.rommFilterFavorites: 'Favoritos',
  AppLocale.rommFilterHasSaves: 'Com saves',
  AppLocale.rommFilterHasStates: 'Com save states',
  AppLocale.rommFilterHasAchievements: 'Com conquistas',
  AppLocale.rommFilterPlayable: 'Jogáveis',
  AppLocale.rommFilterDuplicates: 'Duplicados',
  AppLocale.rommFilterMissing: 'Ficheiros em falta',
  AppLocale.rommFilterChipsLabel: 'Filtros:',
  AppLocale.rommFilterClearAll: 'Limpar filtros',
  AppLocale.rommFilterNoMatches: 'Nenhuma ROM corresponde a estes filtros',
  AppLocale.rommSurpriseMe: 'Surpreende-me',
  AppLocale.rommSurpriseMeEmpty: 'Não há nada para escolher aqui',
  AppLocale.rommSurpriseMeUnsupported:
      'Este servidor RomM é demasiado antigo para uma escolha aleatória',
  ...
  AppLocale.rommMaintenanceRescanConfirm:
      'O RomM vai analisar toda a biblioteca de novo. Pode demorar bastante num servidor grande.',
  AppLocale.rommMaintenanceSyncFolders: 'Sincronizar análise de pastas',
  AppLocale.rommMaintenanceSyncFoldersConfirm:
      'O RomM vai reler as suas pastas de ROM e apanhar ficheiros adicionados ou removidos fora da aplicação.',
  AppLocale.rommMaintenanceCleanup: 'Limpar ROM em falta',
  AppLocale.rommMaintenanceCleanupConfirm:
      'O RomM vai remover as entradas da biblioteca cujos ficheiros já não estão no servidor. Isto não pode ser desfeito.',
  AppLocale.rommMaintenanceRun: 'Executar',
  ...
  AppLocale.neoSyncNetworkError: 'Erro de rede: {error}',
  AppLocale.neoSyncServerError: 'Erro do servidor: {error}',
  AppLocale.neoSyncLoginFailed: 'Falha ao entrar',
  AppLocale.neoSyncRegistrationFailed: 'Falha no registro',
  AppLocale.neoSyncVerificationFailed: 'Falha na verificação',
  ...
  AppLocale.neoSyncPasswordResetFailed: 'Falha ao redefinir a senha',
  AppLocale.neoSyncCheckoutFailed: 'Falha ao criar a sessão de pagamento',
  AppLocale.neoSyncCancelSubscriptionFailed: 'Falha ao cancelar a assinatura',
  AppLocale.neoSyncPlansFailed: 'Falha ao obter os planos',
  AppLocale.rommServerVersionLine: 'Versão do servidor {version}',
  AppLocale.rommPasswordLoginDisabledHint:
      'O login com senha está desativado neste servidor',
  AppLocale.rommPushPlayState: 'Enviar estado de jogo para o RomM',
  AppLocale.rommPushPlayStateHint:
      'Envia ao RomM as alterações de oculto, favorito e última jogada dos jogos vinculados',
  AppLocale.rommLinkSourceHash: 'Vinculado por hash a {name}',
  AppLocale.rommMatchByHash: 'Corresponder por hash',
  AppLocale.rommMatchByHashBusy: 'A corresponder por hash…',
  AppLocale.rommMatchByHashNoMatch:
      'Nenhuma entrada do RomM tem o hash deste ficheiro',
  AppLocale.rommMatchByHashSkipped:
      'Não foi possível calcular o hash do ficheiro: {reason}',
  AppLocale.rommMatchByHashFailed: 'A correspondência por hash falhou',
  AppLocale.rommMatchByHashReasonDisc: 'imagens de disco não são hasheadas',
  AppLocale.rommMatchByHashReasonOversize:
      'o ficheiro é demasiado grande para calcular o hash',
  AppLocale.rommMatchByHashReasonMissing: 'o ficheiro não foi encontrado',
  ...
  AppLocale.rommRemoteBadge: 'No servidor RomM, não baixado',
  AppLocale.rommRemoteRetryBadge:
      'Falha no download, pressione A para tentar novamente',
  AppLocale.rommRemoteDownloadingBadge: 'Baixando {percent}%',
  AppLocale.rommRemoteDownloadingIndeterminate: 'Baixando…',
  AppLocale.rommRemoteCancelDownload: 'Cancelar download',
  AppLocale.rommRemoteDownloadConfirmTitle: 'Baixar do RomM',
  AppLocale.rommRemoteDownloadConfirmBody:
      'Baixar {name} ({size}) para {folder}?',
  AppLocale.rommRemoteSizeUnknown: 'tamanho desconhecido',
  AppLocale.rommRemoteCancelConfirmTitle: 'Cancelar download',
  AppLocale.rommRemoteCancelConfirmBody:
      'Parar o download de {name}? O arquivo parcial será descartado.',
  AppLocale.rommRemoteOfflineNotice:
      'Indisponível offline: não foi possível acessar o servidor RomM',
  AppLocale.rommRemotePlayNow: 'Jogar agora',
  AppLocale.rommRemotePlayLater: 'Mais tarde',
  AppLocale.rommRemoteDownloadReadyTitle: 'Download concluído',
  AppLocale.rommRemoteDownloadReadyBody: '{name} está pronto para jogar.',
  AppLocale.rommRemoteSecondaryState: 'No servidor RomM, não baixado',
  AppLocale.collectionPushRomm: 'Enviar para o RomM',
  AppLocale.collectionRommPushed: 'Enviada para o RomM',
  AppLocale.collectionPushRommOutcome:
      '"{name}" enviada para o RomM: {pushed} enviados, {unlinked} não vinculados',
  AppLocale.collectionPushRommAlreadyExists:
      'Já existe uma coleção com este nome no RomM',
  AppLocale.collectionPushRommDenied:
      'Este login do RomM não pode gravar coleções',
  AppLocale.collectionPushRommFailed:
      'Não foi possível enviar "{name}" para o RomM',
  AppLocale.collectionDeleteRommTitle: 'Excluir também no RomM?',
  AppLocale.collectionDeleteRommBody:
      '"{name}" foi enviada para o RomM. Excluí-la lá também ou manter a cópia do RomM?',
  AppLocale.collectionDeleteRommConfirm: 'Excluir no RomM',
  AppLocale.collectionDeleteRommKeep: 'Manter no RomM',
  AppLocale.collectionUnlinkPushedConfirm:
      'Esta coleção deixará de enviar suas alterações ao RomM. Seus jogos permanecem aqui e no RomM.',
  AppLocale.rommSyncOutcomeCollectionManagedHere:
      'A coleção "{name}" é gerenciada a partir deste dispositivo e não foi atualizada do RomM',
  AppLocale.collectionPushRommMembersQueued:
      '"{name}" está no RomM; os jogos serão enviados quando o servidor estiver acessível',
  AppLocale.neoSyncLoginSuccessfulEmailNotVerified:
      'Login realizado com sucesso, mas seu e-mail ainda não foi verificado',
  AppLocale.neoSyncVerificationEmailSent: 'E-mail de verificação enviado',
  ...
  AppLocale.rommUploadNoPlatform:
      'O RomM não tem uma plataforma para {system}. Crie-a no seu servidor RomM e envie novamente.',
  AppLocale.rommUploadUnknownSystem:
      'O NeoStation não conhece nenhum sistema chamado {system}, por isso não consegue escolher uma plataforma do RomM. Verifique o nome da pasta de ROM.',
  AppLocale.rommUploadAmbiguousPlatform:
      'O RomM tem mais de uma plataforma para {system}: {platforms}. Junte-as no servidor e envie novamente.',
  ...
  AppLocale.rommUploadMetadataPushed: 'Detalhes enviados de {count} jogos',
